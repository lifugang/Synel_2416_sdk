...
//			if((int)com1value.address==cardinfo.track)
			if((int)com1value.address==1 || (int)com1value.address==2)	
			{
				if((int)com1value.itemnum==0x0)
				{
					if(com1value.user_data[0]==0x00)
				...
				while(i<len)
				{
					sno[i]=(int)com1value.user_data[i];
					i++;
				}
				memcpy(cardnum,sno,strlen(sno));
				return (int)com1value.address;
			}
			else 
				return 0;
		...
		case CARD_14443B:	// 20190618 add
			...
			memset(buf,0,sizeof(buf));
			sprintf(buf, "%02X%02X%02X%02X",*card++,*card++,*card++,*card++);
			sprintf((char *)sno,"%u",strtoul((const char *)buf,NULL,16));
			break;			
