...
int get_cmd_response(unsigned char inst)
{
	int i = 0;
	_TData answer;
	struct timespec cmdsendtime;

	clock_gettime(CLOCK_MONOTONIC, &cmdsendtime);

	memset(&answer, 0, sizeof(answer));
	while(1)
	{			
		if(RelativeTimer(cmdsendtime, 200))
		{
			return FALSE;
		}
...
//			 }
		...
		{
//			printf("err answer.instruction==%02X\n",answer.instruction);
			continue;
		}		
...
//unsigned char data[10]={0x33};
	...
	return get_cmd_response(ask.instruction);

}


// mode : 0-wg reader, 1-magic reader
int set_ex_reader_mode(int mode)
{
//	unsigned char mdata[] = {0xAA, 0x01, 0x03, 0x71, 0x01, 0x91, 0xE3, 0x07, 0xA5};
//	unsigned char wdata[] = {0xAA, 0x01, 0x03, 0x71, 0x01, 0x97, 0xE5, 0x0D, 0xA5};
	...
	{
		case 0:
			ask.user_data[0] = 0x97;
			break;
		case 1:
			ask.user_data[0] = 0x91;
			break;
		default:
			ask.user_data[0] = 0x97;
			break;
...
// close antenna 20190214
// type: 0-ID ant, 1-IC ant
// opt: 0-disable, 1-enable
int set_antenna_status(int type, int opt)
{

//	AA 01 04 72 01 A0 00 d6 18 A5 

	_TData ask;
	int ret, redo = 0;
	memset(&ask, 0, sizeof(ask));
	ask.address = 0X01;
	ask.instruction = 0X72;
	ask.nbytes = 0X02;
	ask.itemnum = 0x01;
	...
	{
		case 0:
			ask.user_data[0] = 0xA0;			
			break;
		case 1:
			ask.user_data[0] = 0xA1;
			break;
		default:
			return FALSE;
			break;
	...
	if(opt)
	{
		ask.user_data[1] = 0xFF;
	}
	else
	{
		ask.user_data[1] = 0x00;
	}
...
//unsigned char data[10]={0x33};
	...
	if(get_cmd_response(answer.instruction) == TRUE)
	{
		for (i = 0; i < com1value.nbytes; i++)
		sprintf(ver + i, "%c", com1value.user_data[i]);
