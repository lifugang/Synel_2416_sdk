...
int _input_number( const char* szPrompt, int dwDefault )
{
	...
	printf( "Please input %s:", szPrompt );
...
void _get_finger_quality( int nMilliSecondTimeout )
{
	int		nRet;
...
//		if(RelativeTimer(curtime, nMilliSecondTimeout))
//		{
//			printf( "Timeout\n" );
//			break;
//		}
...
//	fpinfo.sensortype = 0x33;               // superma
	...
	printf( "9 :  get FP list loaded in FP\n" );
	printf( "10 :  fingerprint image to be evaluated, 256*256 dimension\n" );
	printf( "------------------\n" );
	...
	while ( nContinue )
	{
		...
		case 9:
			...
			break;
		case 10:
			printf( "get finger quality\n" );
			_get_finger_quality( 5000 );
			break;
		}
	}
