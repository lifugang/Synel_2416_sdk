...
int main(int argc, char *argv[]){
    ...
    printf("Usage: update mcu baud_table_index \r\n");
    printf("baud table: 0 = 2400,1 = 4800, 2 = 9600,3 = 19200,4 = 38400,5 = 57600,6 = 115200\r\n");
    printf("update mode: 0 = auto 1 = active\r\n");
    printf("update file name\r\n");
    ...
    while(1)
    {
        ...
        key = check_mcu_update();
        if(key == TRUE)
        {
            printf("auto update mcu\n");
            update_mcu(file,0);
            printf("auto update mcu ok\n");
            continue ;
        }
        key=ThreadKeyBoard();
        if(key==-1)	continue;
        ...
        case 12:
            ...
            break;
        case 1:
            printf("active update mcu\n");
            update_mcu(file,1);
            printf("active update mcu ok\n");
            break;
        default:
            p=ReadKey(key);
            if (p){
